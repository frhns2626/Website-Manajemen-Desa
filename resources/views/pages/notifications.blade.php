    ...
    <div class="row">
        @foreach (auth()->user()->notifications as $notification)
            <div class="col-12 mb-3">
                <div class="card">
                    <div class="card-body" style="background-color: rgba(115, 194, 251, {{is_null($notification->read_at) ? '0.2' : ''}});">
                        <div class="row">
                            ...
                            @if (is_null($notification->read_at))
                                <div class="col-2">
                                    <form action="/notification/{{ $notification->id }}/read" method="post">
                                        @csrf
                                        @method('POST')
                                        <button type="submit" class="btn btn-primary btn-sm btn-block">Tandai Dibaca</button>
                                    </form>
                                </div>
                            @endif
                        </div>
